			...
			self.env.cr.execute(f"""
	            CREATE OR REPLACE VIEW {self._table} AS (
	                SELECT 
	                    ...
	                    COALESCE(enc_stats.total_spent, 0) AS total_spent,
	                    COALESCE(enc_stats.pending_amount, 0) AS pending_amount,
	                    ...
	                    (COALESCE(enc_stats.total_spent, 0) > 5000) AS high_value_client,
	                    ...
	                    CASE 
	                        WHEN COALESCE(enc_stats.total_spent, 0) > 10000 THEN 'Platinum'
	                        WHEN COALESCE(enc_stats.total_spent, 0) > 5000 THEN 'Gold'
	                        WHEN COALESCE(enc_stats.total_spent, 0) > 1000 THEN 'Silver'
	                        WHEN COALESCE(enc_stats.total_spent, 0) > 0 THEN 'Bronze'
	                        ELSE 'New'
	                ...
	                -- Encounter Statistics using JSON_CONTAINS or similar approach
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(e.patient_ids, '$[*]')) AS pet_id_json,
	                        COUNT(*) AS encounter_count,
	                        MAX(e.encounter_date) AS last_visit_date,
	                        MIN(e.encounter_date) AS first_visit_date,
	                        SUM(e.total_amount) AS total_spent,
	                        SUM(e.pending_amount) AS pending_amount
	                    FROM vet_encounter_header e
	                    WHERE e.state IN ('in_progress', 'done')
	                      AND e.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(e.patient_ids, '$[*]'))
	                ) enc_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(enc_stats.pet_id_json, '[', ''), ']', ''))

	                -- Vaccination Statistics
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(v.patient_ids, '$[*]')) AS pet_id_json,
	                    ...
	                    WHERE v.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(v.patient_ids, '$[*]'))
	                ) vacc_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(vacc_stats.pet_id_json, '[', ''), ']', ''))

	                -- Boarding Statistics
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(b.patient_ids, '$[*]')) AS pet_id_json,
	                        COUNT(*) AS boarding_count,
	                        MAX(b.check_in_datetime::date) AS last_board_date
	                    FROM vet_boarding_stay b
	                    WHERE b.state != 'cancelled' AND b.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(b.patient_ids, '$[*]'))
	                ) board_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(board_stats.pet_id_json, '[', ''), ']', ''))

	                -- Park Visit Statistics
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(p.patient_ids, '$[*]')) AS pet_id_json,
	                        COUNT(*) AS park_visit_count,
	                        MAX(p.checkin_time::date) AS last_park_date
	                    FROM vet_park_checkin p
	                    WHERE p.state = 'checked_out' AND p.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(p.patient_ids, '$[*]'))
	                ) park_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(park_stats.pet_id_json, '[', ''), ']', ''))

	                -- Membership Statistics
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(m.patient_ids, '$[*]')) AS pet_id_json,
	                        COUNT(*) AS membership_count
	                    FROM vet_pet_membership m
	                    WHERE m.state = 'active' AND m.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(m.patient_ids, '$[*]'))
	                ) memb_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(memb_stats.pet_id_json, '[', ''), ']', ''))

	                -- Appointment Statistics
	                LEFT JOIN (
	                    SELECT 
	                        JSON_UNQUOTE(JSON_EXTRACT(c.patient_ids, '$[*]')) AS pet_id_json,
	                        COUNT(*) AS appointment_count,
	                        MAX(c.start::date) AS last_apt_date
	                    FROM calendar_event c
	                    WHERE c.appointment_status IN ('completed', 'paid') AND c.patient_ids IS NOT NULL
	                    GROUP BY JSON_UNQUOTE(JSON_EXTRACT(c.patient_ids, '$[*]'))
	                ) appt_stats ON FIND_IN_SET(pet.id, REPLACE(REPLACE(appt_stats.pet_id_json, '[', ''), ']', ''))
